...
const CheckoutDetails = ({ loggedInUser, setLoading }) => {
  // populate the checkout details using hook
  ...
  // set the values when the checkoutdetails have updated
  useEffect(() => {
    setEditedDetails(checkOutDetails);
  }, [checkOutDetails]);
  // handle change for each respective field
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setEditedDetails((prevDetails) => ({ ...prevDetails, [name]: value }));
  };
  // allow edit for the specified field
  ...
  const handleSave = async (field) => {
    ...
    // some simple validations for the fields
    ...
    // attempt and update the checkout values and catch any errors
    try {
      await updateCheckoutDetails(
        [{ accountValue: field, value: editedDetails[field] }],
        checkOutDetails.ID
      );
